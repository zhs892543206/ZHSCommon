 ...
 * Date: 16/03/11
 */
public abstract class SearchCommonAdapter<T> extends RecyclerView.Adapter<SearchViewHolder> {
    ...
    public SearchCommonAdapter setOnItemClickListener(OnItemClickListener onItemClickListener) {
    ...
    public SearchCommonAdapter(Context context, int layoutId, List<T> datas) {
        mContext = context;
    ...
    @Override
    public SearchViewHolder onCreateViewHolder(final ViewGroup parent, int viewType) {
        SearchViewHolder viewHolder = SearchViewHolder.get(mContext, null, parent, mLayoutId, -1);
     ...
     * 所以如果使用了{@link HeaderAndFooterWrapperAdapter},建议使用 {@link #setListener(int, SearchViewHolder)} 这个方法，返回的位置是正确的。
     *
     * @param parent
     * @param viewHolder
     * @param viewType
     */
    @Deprecated
    protected void setListener(final ViewGroup parent, final SearchViewHolder viewHolder, int viewType) {
    ...
    //add by zhangxutong 2016 08 05 begin 点击事件为了兼容HeaderView FooterView 的Adapter，所以在OnBindViewHolder里，其实性能没有onCreate好
    protected void setListener(final int position, final SearchViewHolder viewHolder) {
    ...
    public abstract void convert(SearchViewHolder holder, T t);
